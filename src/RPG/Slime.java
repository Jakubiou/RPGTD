...
public class Slime extends Enemy {
    ...
    public Slime(int x, int y, int hp) {
        ...
        try {
            for (int i = 0; i < 6; i++) {
                slimeTexturesRight[i] = ImageIO.read(new File("res/rpg/enemy/Enemy_slime" + (i + 1) + ".png"));
            }
            slimeTexturesLeft[0] = slimeTexturesRight[0];
            for (int i = 1; i < 6; i++) {
                slimeTexturesLeft[i] = ImageIO.read(new File("res/rpg/enemy/Enemy_slime" + (i + 6) + ".png"));
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
